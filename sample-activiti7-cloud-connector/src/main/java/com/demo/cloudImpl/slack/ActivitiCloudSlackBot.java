package com.demo.cloudImpl.slack;
...
import com.demo.cloudImpl.ProcessRuntimeChannels;
    ...
    public void onReceiveMessage(WebSocketSession session, Event event, Matcher matcher) {
        if (!matcher.group(0).isEmpty()) {
            StartProcessPayload startProcessInstanceCmd = ProcessPayloadBuilder.start()
                    ...
                    .build();
            System.out.println(startProcessInstanceCmd.toString());
            System.out.println(MessageBuilder.withPayload(startProcessInstanceCmd).build().toString());
            System.out.println(processRuntimeChannels.myCmdProducer().send(MessageBuilder.withPayload(startProcessInstanceCmd).build()));
