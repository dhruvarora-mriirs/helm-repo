...
public class SlackConnector {
	...
	public void notifySlackChannel(IntegrationRequest event) throws InterruptedException {
		...
		String var1 = SlackConnector.class.getSimpleName() + " was called for instance "
				+ event.getIntegrationContext().getProcessInstanceId();

		// @TODO: add your code here
		try {
			bot.pushMessageToChannel("editors", "@everyone There is new content to review: "
					+ (String) event.getIntegrationContext().getInBoundVariables().get("content"));
		} catch (IOException e) {
			e.printStackTrace();
		}
